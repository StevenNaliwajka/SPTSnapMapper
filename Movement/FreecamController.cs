...
namespace SPTSnapMaper.Movement
{
     public class FreecamController : MonoBehaviour
     {
          ...
          public void Start()
          {
               ...
               if (_mainCamera == null)
               {
                    SPTSnapMaperPlugin.Logger.LogError("Failed to locate main camera");
                    return;
               }
               ...
               if (_playerUi == null)
               {
                    SPTSnapMaperPlugin.Logger.LogError("Failed to locate player UI");
                    return;
               }
               ...
               {
                    SPTSnapMaperPlugin.Logger.LogError("Failed to add Freecam script to Camera");
               }
               ...
               {
                    SPTSnapMaperPlugin.Logger.LogError("Failed to locate GamePlayerOwner");
               }
          }
          ...
          public void Update()
          {
               if (SPTSnapMaperPlugin.ToggleUi.Value.IsDown())
               {
                    ToggleUi();
               }

               if (SPTSnapMaperPlugin.ToggleFreecamMode.Value.IsDown())
               {
                    ToggleCamera();
               }

               if (SPTSnapMaperPlugin.ToggleFreecamControls.Value.IsDown())
               {
                    ToggleCameraControls();
               }

               if (SPTSnapMaperPlugin.TeleportToCamera.Value.IsDown())
               {
                    MovePlayerToCamera();
               }
          }
          ...
          private void SetPlayerToFreecamMode(Player localPlayer)
          {
               ...
               {
                    SPTSnapMaperPlugin.Logger.LogError("Failed to get the PlayerBody field");
               }

               // Instead of Detouring, just turn off _gamePlayerOwner which takes the input
               _gamePlayerOwner.enabled = false;

               if (SPTSnapMaperPlugin.CameraRememberLastPosition.Value && _lastPosition != null && _lastRotation != null)
               {
          ...
          private void SetPlayerToFirstPersonMode(Player localPlayer)
          {
               ...
               if (SPTSnapMaperPlugin.CameraRememberLastPosition.Value)
               {
